...
P01<-function(){
  ...
  for(i in 1:n){
    ...
    df$Extra<-substr(df$Extra,df$nx+2,999)
...
#           "Matrix","CAS","Substance",
#           "SubstanceShort","Species",
#           "Sex","Subcomponent")
...
dfP01 <- P01() %>% 
  select(keep) %>%
  rename(Tissue=Subcomponent)

write.table(dfP01,file="P01.txt",row.names=FALSE,quote=FALSE,sep='\t')
